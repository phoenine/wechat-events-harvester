            ...
            <a-space>
              ...
              <a-space-item v-else>
                <a-dropdown trigger="click" position="br">
                  ...
                  <template #content>
                    <a-doption @click="clear_expired_articles">
                      清理过期文章
                    </a-doption>
...
const showRefreshModal = () => (refreshModalVisible.value = true);
...
const clear_expired_articles = () => {
  Modal.confirm({
    title: "确认清理过期文章",
    content: "将删除15天前的所有文章（按发布时间）。该操作不可恢复。",
    okText: "确认",
    cancelText: "取消",
    onOk: async () => {
      fullLoading.value = true;
    ...
    onCancel: () => {
      Message.info("已取消清理过期文章");
    },
  });
};

const refresh = () => showRefreshModal();

const showAddModal = () => router.push("/add-subscription");

const handleAddSuccess = () => fetchArticles();
...
const currentArticle = ref({ title: "", content: "", time: "", url: "" });
...
const fetchMpList = async () => {
  ...
  try {
    ...
    mpList.value = res.list.map((item: any) => ({
