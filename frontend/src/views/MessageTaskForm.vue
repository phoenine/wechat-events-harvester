...
const cronPickerRef = ref<InstanceType<typeof cronExpressionPicker> | null>(
  null
);
    ...
    {% endfor %}
        ...
        <a-form-item label="公众号" field="mps_id">
          <a-space>
            <a-input
              :model-value="
                (formData.mps_id || []).map((mp) => mp.id.toString()).join(',')
              "
      ...
      >
        ...
        <template #footer>
          <a-button type="primary" @click="showCronPicker = false"
            >确定</a-button
          >
        </template>
      </a-modal>
      ...
      >
        ...
        <template #footer>
          <a-button type="primary" @click="showMpSelector = false"
            >确定</a-button
          >
        </template>
      </a-modal>
