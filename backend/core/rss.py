...
class RSS:
    ...
    def add_logo_prefix_to_urls(self, text: str) -> str:
        ...
        try:
            pattern = re.compile(
                r'(<img[^>]*src=["\'])(?!\/static\/res\/logo\/)([^"\']*)', re.IGNORECASE
            )
    ...
    def generate_rss(
        ...
        for rss_item in rss_list:
            ...
            ET.SubElement(item, "pubDate").text = self.datetime_to_rfc822(
                str(rss_item["updated"])
            )

        # 生成XML字符串(添加声明和美化输出)
        tree_str = '<?xml version="1.0" encoding="utf-8"?>\r\n' + ET.tostring(
            rss, encoding="utf-8", method="xml", short_empty_elements=False
        ).decode("utf-8")
    ...
    def generate_atom(
        ...
        # Use timezone-aware now (CST/UTC+8) so %z shows +0800
        ET.SubElement(feed, "updated").text = datetime.now(
            timezone(timedelta(hours=8))
        ).strftime("%a, %d %b %Y %H:%M:%S %z")
        ...
        for rss_item in rss_list:
            ...
            ET.SubElement(entry, "updated").text = self.datetime_to_rfc822(
                str(rss_item["updated"])
            )
        ...
        # 生成XML字符串
        tree_str = '<?xml version="1.0" encoding="utf-8"?>\r\n' + ET.tostring(
            feed, encoding="utf-8", method="xml"
        ).decode("utf-8")
    ...
    def generate_json(
            ...
            "items": [
                {
                    ...
                    "updated": (
                        ...
                        if isinstance(item["updated"], datetime)
                        else item["updated"]
                    ),
        ...
        return json.dumps(
            result, ensure_ascii=False, indent=2, default=self.serialize_datetime
        )
    ...
    def generate(
        ...
        if ext in ("rss", "xml"):
            ...
            return self.generate_by_template(
                rss_list,
                template,
                title=title,
                link=link,
