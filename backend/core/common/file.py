...
class FileCrypto:
    ...
    def __init__(self, password: str):
        ...
        """
        self.key = (
            hashlib.sha256(password.encode()).digest() if password != None else None
        )
...
if __name__ == "__main__":
    ...
    logger.info(f"原始数据: {original_text}")

    # 加密
    encrypted_data = crypto.encrypt(original_text.encode("utf-8"))
    logger.info(f"加密后数据: {b64encode(encrypted_data).decode('utf-8')}")
