    ...
    def init(self, con_str: str) -> None:
        ...
        try:
            ...
            self.engine = create_engine(
                con_str,
                pool_size=5,  # 最小空闲连接数（Supabase推荐5-10）
                max_overflow=10,  # 允许的最大溢出连接数（Supabase限制）
                pool_timeout=30,  # 获取连接时的超时时间（秒）
                echo=False,
                pool_recycle=300,  # 连接池回收时间（秒）- Supabase推荐5分钟
                isolation_level="READ COMMITTED",  # PostgreSQL标准隔离级别
                #  isolation_level="AUTOCOMMIT",  # 仅用于SQLite
                #  query_cache_size=0,
                connect_args=(
