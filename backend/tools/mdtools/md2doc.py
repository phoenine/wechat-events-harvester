...
class MarkdownToWordConverter:
    ...
    def convert_file(
        self, markdown_file: str, output_file: str, document_title: Optional[str] = None
    ) -> bool:
        """
        ...
        try:
            ...
            while i < len(lines):
                ...
                if self._is_heading(line):
                    ...
                    elif self.config.get("remove_images", False) and re.search(
                        r"!\[(.*?)\]\((.*?)\)", line.strip()
                    ):
                        # 如果配置为去除图片，则跳过包含图片的行
                        pass
    ...
    def convert_text(
        self, markdown_text: str, output_file: str, document_title: Optional[str] = None
    ) -> bool:
        """
    ...
    def convert_to_document(
        self, markdown_text: str, document_title: Optional[str] = None
    ) -> Optional[Document]:
        """
    ...
    def _process_inline_formatting(self, text: str) -> List[Dict[str, Any]]:
        ...
        if current_text:
            parts.append(
                {"text": current_text, "bold": False, "italic": False, "code": False}
            )

        return (
            parts
            if parts
            else [{"text": text, "bold": False, "italic": False, "code": False}]
        )
        ...
        try:
            ...
            try:
                ...
                if width_inches <= max_width and height_inches <= max_height:
                    ...
                    self.logger.info(
                        f"添加原尺寸图片成功: {img_url} ({width}x{height}px, {width_inches:.2f}x{height_inches:.2f}in)"
                    )
                else:
                    # 图片较大，需要缩放
                    ...
                    if is_landscape:
                        ...
                        self.logger.info(
                            f"添加缩放横图成功: {img_url} ({width}x{height}px)"
                        )
                    else:
                        # 竖图：固定高度，宽度自动调整
                        run.add_picture(converted_path, height=Inches(6))
                        self.logger.info(
                            f"添加缩放竖图成功: {img_url} ({width}x{height}px)"
                        )
                ...
                if converted_path != local_path:
                    try:
                        ...
                        self.logger.warning(
                            f"删除转换后的临时文件失败: {converted_path}, 错误: {str(e)}"
                        )
            ...
            try:
                ...
                self.logger.warning(
                    f"删除临时图片文件失败: {local_path}, 错误: {str(e)}"
                )
...
def main():
    ...
    parser.add_argument(
        "input",
        nargs="?",
        help="输入的 Markdown 文件路径（可选，不提供则从标准输入读取）",
    )
    ...
    if args.content:
        # 直接使用提供的内容
        success = converter.convert_content_to_file(
            args.content, args.output, args.title
        )
        if success:
