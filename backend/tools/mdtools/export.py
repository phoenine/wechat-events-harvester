...
def process_single_article(
    ...
    if export_docx and document or export_md or export_json or export_csv or export_pdf:
        ...
        name = (
            datetime.fromtimestamp(art.publish_time).strftime("%Y%m%d")
            + "_"
            + art.title
        )
        ...
        try:
            ...
            if export_csv and writer:
                writer.writerow(
                    [
                        art.title,
                        art.url,
                        datetime.fromtimestamp(art.publish_time).strftime(
                            "%Y-%m-%d %H:%M:%S"
                        ),
                    ]
                )
            ...
            if export_md:
                exported_files.append("MD")
            if export_docx and document:
                exported_files.append("DOCX")
            if export_pdf and document:
...
def export_md_to_doc(
    ...
    if record_count > 0:
        ...
        try:
            ...
            sys_notice(
                f"文章导出完成！共处理 {record_count} 篇文章。下载链接: [点击下载]({download_link})"
            )
        except Exception as e:
