...
from core.wx.cfg import wx_cfg
...
class WXArticleFetcher:
    ...
    def get_article_content(self, url: str) -> Dict:
        ...
        # 注入已登录的微信 Cookie（如存在），减少环境异常拦截
        try:
            cookies_str = wx_cfg.get("cookie", "")
            if cookies_str:
                ...
                for kv in pairs:
                    if "=" in kv:
                        ...
                        cookies.append({"name": name.strip(), "value": value.strip(), "url": "https://mp.weixin.qq.com"})
                if cookies:
        ...
        try:
            ...
            # 优先等待正文容器出现
            try:
                page.wait_for_selector("#js_content, #js_article, body", timeout=self.wait_timeout)
            except Exception:
                pass
