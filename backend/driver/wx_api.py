...
class WeChatAPI:
    ...
    def get_qr_code(
        self, callback: Optional[Callable] = None, notice: Optional[Callable] = None
    ) -> Dict[str, Any]:
        """
        获取登录二维码
    ...
    def _get_account_list(self) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            headers = {
                "accept": "*/*",
                "accept-language": "zh-CN,zh;q=0.9,en;q=0.8",
                "sec-ch-ua": '"Not?A_Brand";v="99", "firefox";v="130"',
                "sec-ch-ua-mobile": "?0",
    ...
    def login_with_token(
        self, token: str = "", cookies: Optional[Dict[str, str]] = None
    ) -> bool:
        """
        使用token登录
    ...
    def GetCode(self, CallBack=None, Notice=None):
        ...
        self.thread = ThreadManager(
            target=self.get_qr_code, args=(CallBack, Notice)
        )  # 传入函数名
...
def get_qr_code(
    callback: Optional[Callable] = None, notice: Optional[Callable] = None
) -> Dict[str, Any]:
    """
    获取登录二维码（全局函数）
