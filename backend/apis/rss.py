...
async def get_rss_source(
    ...
    return await get_mp_articles_source(
        request=request, feed_id=feed_id, limit=limit, offset=offset, is_update=True
    )
...
async def update_rss_feeds(
    ...
    return await get_rss_feeds(
        request=request, limit=limit, offset=offset, is_update=True
    )
        ...
        rss_list = [
            {
                ...
                "updated": (
                    feed.created_at
                    if getattr(feed.created_at, "tzinfo", None) is not None
                    else feed.created_at.replace(tzinfo=cst)
                ).isoformat(),
            }
            for feed in feeds
        ]
    ...
    try:
        ...
        if feed_id not in ["all", None]:
            ...
            if tag_id is not None:
                ...
                if tags:
                    mps_ids = (
                        [str(mp["id"]) for mp in json.loads(tags.mps_id)]
                        if tags.mps_id
                        else []
                    )
        ...
        rss_list = [
            {
                ...
                "link": (
                    f"{rss_domain}rss/feed/{article.id}"
        ...
        for _feed, article in articles:
            content_data = {
                ...
                "content_md": getattr(article, "content_md", None),
        ...
        rss_xml = rss.generate(
            rss_list,
            ext=ext,
            title=f"{feed.mp_name}",
            link=rss_domain,
            description=feed.mp_intro,
            image_url=feed.mp_cover,
            template=template,
        )
