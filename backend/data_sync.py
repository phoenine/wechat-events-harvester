    ...
    def _map_types_for_database(self, model):
        ...
        for column in model.__table__.columns:
            ...
            if "sqlite" in self.db_url:
                ...
                if (
                    (
                        ...
                        and column.type.__visit_name__ == "MEDIUMTEXT"
                    )
                    or "MEDIUMTEXT" in type_str
                    or getattr(column.type, "__class__", None).__name__ == "MEDIUMTEXT"
                ):
                    from sqlalchemy import Text

                    column.type = Text()
                    self.logger.debug(
                        f"已将列 {column.name} 的类型从 MEDIUMTEXT 映射为 Text"
                    )
            ...
            # PostgreSQL类型映射
                ...
                # LONGTEXT映射为TEXT
                if (
                    "LONGTEXT" in type_str
                    or getattr(column.type, "__class__", None).__name__ == "LONGTEXT"
                ):
                    from sqlalchemy import Text

                    column.type = Text()
                    self.logger.debug(
                        f"已将列 {column.name} 的类型从 LONGTEXT 映射为 Text"
                    )

                # TINYINT映射为SMALLINT
                if (
                    "TINYINT" in type_str
                    or getattr(column.type, "__class__", None).__name__ == "TINYINT"
                ):
                    from sqlalchemy import SmallInteger

                    column.type = SmallInteger()
                    self.logger.debug(
                        f"已将列 {column.name} 的类型从 TINYINT 映射为 SmallInteger"
                    )
        ...
        try:
            with self.engine.begin() as conn:
                ...
                if "postgresql" in self.db_url or "postgres" in self.db_url:
                    ...
                    self.logger.info(
                        f"当前用户: {user_info[0]}, 数据库: {user_info[1]}, Schema: {user_info[2]}"
                    )
                    ...
                    if not perms[0]:  # 没有CREATE权限
                        ...
                        self.logger.info(
                            f"GRANT CREATE ON SCHEMA public TO {user_info[0]};"
                        )
                        return False
                    ...
                    if not perms[1]:  # 没有USAGE权限
                        ...
                        self.logger.info("请联系数据库管理员执行以下命令:")
                        self.logger.info(
                            f"GRANT USAGE ON SCHEMA public TO {user_info[0]};"
                        )
                        return False
                        ...
                        for col_name, model_col in model_columns.items():
                            if col_name not in existing_columns:
                                ...
                                try:
                                    with self.engine.begin() as conn:
                                        if (
                                            "postgresql" in self.db_url
                                            or "postgres" in self.db_url
                                        ):
                                            # PostgreSQL语法
                                            conn.execute(
                                                text(
                                                    f'ALTER TABLE "{table_name}" ADD COLUMN "{col_name}" {model_col.type}'
                                                )
                                            )
                                        else:
                                            # SQLite和MySQL语法
                                    ...
                                    self.logger.info(
                                        f"新增字段: {table_name}.{col_name}"
                                    )
                                except SQLAlchemyError as e:
                                    self.logger.error(
                                        f"添加字段 {table_name}.{col_name} 失败: {e}"
                                    )
